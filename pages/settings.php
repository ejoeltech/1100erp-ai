...
<div id="content-maintenance" class="tab-content hidden">
    ...
    <div class="space-y-8 max-w-4xl">
        ...
        <div class="bg-slate-50 border border-slate-200 rounded-lg p-6">
            ...
            <div class="flex flex-col md:flex-row gap-6 items-start">
                <div class="flex-1">
                    <div id="git-status-display" class="mb-4">
                        <div class="flex items-center gap-3 mb-2">
                            <span id="git-status-badge" class="px-3 py-1 bg-gray-200 text-gray-700 text-xs font-bold rounded-full uppercase tracking-wider">
                                Checking...
                            </span>
                            <span id="git-branch-info" class="text-sm text-gray-500 font-mono"></span>
                        </div>
                        <p id="git-latest-msg" class="text-sm text-gray-600 italic"></p>
                    </div>

                    <div id="git-warning" class="hidden mb-4 p-3 bg-red-50 border border-red-100 rounded text-red-700 text-xs">
                        <strong>Warning:</strong> Local changes detected. They will be stashed and reapplied during update.
                    </div>

                    <div class="flex gap-3">
                        <button type="button" onclick="checkGitStatus()" id="btn-git-check"
                            class="px-4 py-2 bg-slate-100 text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-200 font-semibold text-sm flex items-center gap-2">
                            Refresh Status
                        </button>
                        <button type="button" onclick="pullGitUpdates()" id="btn-git-pull"
                            class="hidden px-6 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 font-semibold shadow-sm text-sm flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="w-full md:w-80 p-4 bg-white rounded border border-slate-100">
                    <div class="text-xs text-gray-500 uppercase tracking-widest font-bold mb-2">System Info</div>
                    <div class="space-y-1">
                        <div class="flex justify-between text-xs">
                            <span class="text-gray-400">Current Commit:</span>
                            <span id="git-local-hash" class="font-mono text-gray-700">-</span>
                        </div>
                        <div class="flex justify-between text-xs">
                            <span class="text-gray-400">Remote Commit:</span>
                            <span id="git-remote-hash" class="font-mono text-gray-700">-</span>
                        </div>
                        <div class="flex justify-between text-xs pt-1 border-t border-slate-50 mt-1">
                            <span class="text-gray-400">Last Checked:</span>
                            <span id="git-last-checked" class="text-gray-700">-</span>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Detailed Logs (Expanded on Click) -->
            <div id="git-logs-container" class="hidden mt-6 pt-4 border-t border-slate-100">
                <h5 class="text-xs font-bold text-gray-400 uppercase mb-2">Execution Logs</h5>
                <pre id="git-logs" class="bg-slate-900 text-slate-300 p-4 rounded text-xs font-mono overflow-x-auto max-h-48"></pre>
            </div>
        </div>
...
<script>
    ...
    function checkGitStatus() {
        ...
        const branchMsg = document.getElementById('git-branch-info');
        const latestMsg = document.getElementById('git-latest-msg');
        const pullBtn = document.getElementById('btn-git-pull');
        const localHash = document.getElementById('git-local-hash');
        const remoteHash = document.getElementById('git-remote-hash');
        const lastChecked = document.getElementById('git-last-checked');
        const warning = document.getElementById('git-warning');

        badge.innerHTML = 'Checking...';
        badge.className = 'px-3 py-1 bg-gray-200 text-gray-700 text-xs font-bold rounded-full uppercase tracking-wider animate-pulse';

        fetch('../api/system/git-status.php')
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    branchMsg.innerText = `Branch: ${data.branch}`;
                    localHash.innerText = data.local_commit;
                    remoteHash.innerText = data.remote_commit;
                    lastChecked.innerText = data.last_checked;
                    latestMsg.innerText = data.latest_message;

                    if (data.update_available) {
                        badge.innerText = 'Update Available';
                        badge.className = 'px-3 py-1 bg-amber-500 text-white text-xs font-bold rounded-full uppercase tracking-wider';
                        pullBtn.classList.remove('hidden');
                    } else {
                        badge.innerText = 'Up to Date';
                        badge.className = 'px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-full uppercase tracking-wider';
                    ...
                    badge.className = 'px-3 py-1 bg-red-500 text-white text-xs font-bold rounded-full uppercase tracking-wider';
    ...
    function pullGitUpdates() {
        if (!confirm('This will pull updates from GitHub. Local changes will be preserved via stash. Proceed?')) return;

        const btn = document.getElementById('btn-git-pull');
        const logContainer = document.getElementById('git-logs-container');
        const logPre = document.getElementById('git-logs');
        ...
        fetch('../api/system/git-pull.php')
            ...
            .then(data => {
                btn.disabled = false;
                btn.innerHTML = 'Apply GitHub Updates';
                
                logContainer.classList.remove('hidden');
                logPre.innerText = data.logs.join("\n") + (data.error ? "\nError: " + data.error : "");
